...
class AvailabilityController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        while ($start->copy()->addMinutes((int) $data['slot_duration'])->lessThanOrEqualTo($end)) {
            ...
            if (!$exists) {
                $doctor->availabilities()->create(['available_date' => $data['available_date'], 'available_time' => $start->format('H:i:s')]);
                $created++;
            }
        ...
        if ($created === 0)
            return back()->withInput()->with('error', 'All slots in this period already exist.');
    ...
    {
        if ($availability->doctor_id !== auth()->user()->doctor->id || $availability->is_booked)
            abort(403);
        $availability->delete();
